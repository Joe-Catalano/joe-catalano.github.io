/**
 * Joseph Catalano Portfolio - Enhanced Script (REVISED DEBUG - Cursor Removed)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (document.querySelector('.hero-title') && document.querySelector('#typing-effect') && document.querySelector('.hero-description')) {
        heroTl
            ...
            .to('#typing-effect', { duration: 2.5, text: { value: "Software Engineering Student | Creative Technologist", delimiter: " " }, ease: "none" }, "-=0.5")
    ...
    try {
        ...
        createFadeUpAnimation('#contact .contact-container', '#contact .contact-subtitle, #contact .contact-methods .contact-card, #contact .contact-location', 0.15);
        gsap.utils.toArray('.timeline-item').forEach((item, index) => {
             const direction = index % 2 === 0 ? 'right' : 'left';
             gsap.from(item.querySelector('.timeline-content'), {
                 scrollTrigger: { trigger: item, start: "top 90%", toggleActions: "play none none none" },
                 opacity: 0, x: direction === 'left' ? -60 : 60, duration: 0.8, ease: "power2.out",
             });
             gsap.from(item, {
    ...
    projectItems.forEach(item => { // Ensure initial state correct for display filtering
        item.style.opacity = 1; item.style.transform = 'none'; item.style.display = 'flex';
    });
     filterButtons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            setTimeout(() => { ScrollTrigger.refresh(); console.log("ScrollTrigger refreshed after filtering."); }, 50);
        });
    });
    ...
    modalTriggers.forEach(trigger => {
        trigger.addEventListener('click', () => { const modalId = trigger.getAttribute('data-modal-target'); const modal = document.getElementById(modalId); if (modal) openModal(modal); });
        trigger.addEventListener('keydown', (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); const modalId = trigger.getAttribute('data-modal-target'); const modal = document.getElementById(modalId); if (modal) openModal(modal); } });
    });
    modals.forEach(modal => { modal.addEventListener('click', (e) => { if (e.target === modal) closeModal(modal); }); });
    modalCloses.forEach(button => { button.addEventListener('click', () => { const modal = button.closest('.modal'); if (modal) closeModal(modal); }); });
    ...
    function openModal(modal) {
        ...
        modal.classList.add('active'); body.style.overflow = 'hidden';
        setTimeout(() => {
            ...
            if (firstFocusable) { firstFocusable.focus(); console.log("Focused element in modal:", firstFocusable); }
        }, 100);
    }
    function closeModal(modal) {
        console.log(`Closing modal: #${modal.id}`);
        modal.classList.remove('active'); body.style.overflow = '';
        if(lastFocusedElement) { console.log("Returning focus to:", lastFocusedElement); try { lastFocusedElement.focus(); } catch (e) { console.warn("Could not return focus:", e); } }
    }
    ...
    if (particlesContainer) {
        if (useParticleBg && typeof particlesJS !== 'undefined') { console.log("Initializing particles.js background."); particlesJS('particles-js', { /* config */ }); }
        else { console.log("Particle background disabled or particles.js library not loaded."); particlesContainer.style.display = 'none'; }
    }
    ...
    // --- === 11. Refresh ScrollTrigger on Load/Resize === ---
     ...
     window.addEventListener('load', () => {
        ...
        setTimeout(() => {
            ScrollTrigger.refresh(); console.log("ScrollTrigger refreshed after load.");
            let topSectionId = 'hero'; let minTop = window.innerHeight;
            document.querySelectorAll('main section[id]').forEach(sec => { const rect = sec.getBoundingClientRect(); if (rect.top >= -navbarHeight - 50 && rect.top < minTop) { minTop = rect.top; topSectionId = sec.id; } });
            setActiveLink(topSectionId); console.log("Initial active link set after load refresh.");
        }, 300);
     });
    let resizeTimeout;
    window.addEventListener('resize', () => { clearTimeout(resizeTimeout); resizeTimeout = setTimeout(() => { console.log("Window Resized. Refreshing ScrollTrigger."); ScrollTrigger.refresh(); }, 250); });
